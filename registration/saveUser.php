...
if (!empty($row_login[0])) {
        ...
        if ($response->success) {
            ...
            echo "<script>alert('Подтвердите, что вы не робот.'); location.pathname='/DBlog/registration/registration.php'</script>";
    }

mysqli_close($link);

if ($result2 == 'TRUE') {
        echo "<script>alert('Вы успешно зарегистрированы! Теперь вы можете зайти на сайт.'); location.pathname='/DBlog/authorization/authorization.php'</script>";
    } else {
